...
/// Component for square root operations, using `SimdBackend` with fallback to `CpuBackend` for small traces.
pub type SqrtComponent = FrameworkComponent<SqrtEval>;

/// Defines the AIR for the sqrt component.
pub struct SqrtEval {
    log_size: u32,
...
impl SqrtEval {
    /// Creates a new `SqrtEval` instance from a claim and lookup elements.
    pub fn new(claim: &SqrtClaim, lookup_elements: NodeElements) -> Self {
        Self {
...
impl FrameworkEval for SqrtEval {
    ...
    /// Evaluates the AIR constraints for the sqrt operation.
