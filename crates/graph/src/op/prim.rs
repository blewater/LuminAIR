use luminair_air::{
    components::{
        ...
        sqrt::table::{SqrtColumn, SqrtTable, SqrtTableRow},
    },
    pie::NodeInfo,
};
use luminal::{
...
/// Implements element-wise square root for LuminAIR.
#[derive(Debug, Clone, Default, PartialEq)]
struct LuminairSqrt {}

impl LuminairSqrt {
    /// Creates a new `LuminairSqrt` instance.
    pub fn new() -> Self {
        Self {}
    }
}

impl LuminairOperator<SqrtColumn, SqrtTable> for LuminairSqrt {
    /// Processes input tensor, generating a trace, claim, and output tensor.
    fn process_trace(
        &mut self,
        inp: Vec<(InputTensor, ShapeTracker)>,
        ...
        for (idx, out) in out_data.iter_mut().enumerate() {
            let input_val = get_index(input, &expr, &mut stack, idx);
            // Get sqrt value and remainder from NumerAIR implementation
