use crate::{
    components::{InteractionClaim, NodeElements, SqrtClaim, TraceColumn, TraceError, TraceEval},
...
/// Represents the trace for the Sqrt component, containing the required registers for its
/// constraints.
#[derive(Debug, Default, PartialEq, Eq, Clone, serde::Serialize, serde::Deserialize)]
pub struct SqrtTable {
    /// A vector of [`SqrtTableRow`] representing the table rows.
    pub table: Vec<SqrtTableRow>,
}

/// Represents a single row of the [`SqrtTable`]
#[derive(Debug, Default, PartialEq, Eq, Clone, serde::Serialize, serde::Deserialize)]
pub struct SqrtTableRow {
...
impl SqrtTable {
    /// Creates a new, empty [`SqrtTable`].
    pub fn new() -> Self {
        Self::default()
    }

    /// Adds a new row to the Sqrt Table.
    pub fn add_row(&mut self, row: SqrtTableRow) {
        self.table.push(row);
    }
    ...
    pub fn trace_evaluation(&self) -> Result<(TraceEval, SqrtClaim), TraceError> {
        ...
        // Create trace
        let mut trace = Vec::with_capacity(SqrtColumn::count().0);
        ...
        assert_eq!(trace.len(), SqrtColumn::count().0);

        Ok((trace, SqrtClaim::new(log_size)))
    }
}

/// Enum representing the column indices in the Sqrt trace.
...
impl SqrtColumn {
    /// Returns the index of the column in the Sqrt trace.
...
/// Generates the interaction trace for the Sqrt component using the main trace and lookup elements.
pub fn interaction_trace_evaluation(
    ...
    let input_main_col = &main_trace_eval[SqrtColumn::Input.index()].data;
    let input_id_col = &main_trace_eval[SqrtColumn::InputId.index()].data;
    let input_mult_col = &main_trace_eval[SqrtColumn::InputMult.index()].data;
